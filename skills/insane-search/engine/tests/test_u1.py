...
def t_validator_xml_edge_cases():
    # Valid XML with content-type
    r1 = _Resp(200, "<root><item>Hello</item></root>", headers={"Content-Type": "application/xml"})
    v1 = validate(r1)
    assert v1.verdict == Verdict.WEAK_OK, v1.verdict

    # Valid XML without content-type but with XML declaration
    r2 = _Resp(200, '<?xml version="1.0"?><root><item>text</item></root>')
    v2 = validate(r2)
    assert v2.verdict == Verdict.WEAK_OK, v2.verdict

    # Empty XML (e.g. <root/>)
    r3 = _Resp(200, "<root/>", headers={"Content-Type": "application/xml"})
    v3 = validate(r3)
    assert v3.verdict == Verdict.WEAK_OK, v3.verdict
    assert v3.reasons == ["xml_ok"], v3.reasons

    # Whitespace-only empty XML (e.g. <root>  \n  </root>)
    r4 = _Resp(200, "<root>  \n  </root>", headers={"Content-Type": "application/xml"})
    v4 = validate(r4)
    assert v4.verdict == Verdict.WEAK_OK, v4.verdict
    assert v4.reasons == ["xml_ok"], v4.reasons

    # XML with Unicode BOM prefix (e.g. \ufeff<?xml version="1.0"?><root><item>text</item></root>)
    r5 = _Resp(200, '\ufeff<?xml version="1.0"?><root><item>text</item></root>')
    v5 = validate(r5)
    assert v5.verdict == Verdict.WEAK_OK, v5.verdict

    # Invalid XML falling back to HTML heuristic validation
    r6 = _Resp(200, "<root><item>text</root>", headers={"Content-Type": "application/xml"})
    v6 = validate(r6)
    # Falls through to HTML validation. Since it's short and incomplete/no-visible-text, it becomes Verdict.CHALLENGE.
    assert v6.verdict == Verdict.CHALLENGE, v6.verdict
    print("  ✓ XML validation edge cases passed")
...
ALL = [
    ...
    ("validator_xml_edge_cases", t_validator_xml_edge_cases),
]
