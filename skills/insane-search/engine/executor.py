...
def _node_cmd() -> str:
    """Discover node binary across macOS, Linux, and Windows environments."""
    n = shutil.which("node")
    if n:
        return n
    n_win = shutil.which("node.exe")
    if n_win:
        return n_win
...
def _pick_executor(capabilities: list[str], device_class: str) -> str:
    """Choose executor based on profile capabilities tags.

    needs_real_tls_stack + needs_js_exec  → playwright_real_chrome.js
    needs_js_exec only                    → playwright_mcp
    needs_mobile_context (+ real_tls)     → playwright_mobile_chrome.js
    """
    if "needs_mobile_context" in capabilities:
        if "needs_real_tls_stack" in capabilities or _node_available():
            return "playwright_mobile_chrome"
        return "playwright_mcp_mobile"
    if "needs_real_tls_stack" in capabilities:
        return "playwright_real_chrome"
    if "needs_js_exec" in capabilities:
        return "playwright_mcp"
    return "playwright_real_chrome"  # safest general fallback


def _get_execution_env() -> dict[str, str]:
    """Build environment PATH injected with cross-platform Node candidate directories."""
    env = os.environ.copy()
    additional_paths = []
    if os.name == "nt":
    ...
    valid_paths = [p for p in additional_paths if os.path.isdir(p)]
    if valid_paths:
        env["PATH"] = path_sep.join(valid_paths) + path_sep + existing_path
    return env


def _run_node_template(template: str, args: dict, timeout: int = 60) -> tuple[int, str, str]:
    """Execute a JS script template with JSON payload over stdin.
...
def run_playwright_fallback(
    ...
    *,
    ...
    if choice.startswith("playwright_mcp"):
        att.error = (
            "Playwright MCP must be invoked from the agent session — "
