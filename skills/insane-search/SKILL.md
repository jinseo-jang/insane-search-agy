...
**Step 0 — run once at the very start, before anything else:** run `bash "setup/setup.sh" ask` (from the insane-search root directory). If its output starts with `STAR_ASK`, ask the user once (via `ask_question` or standard text prompt), with the question and options phrased **in the user's language**: prefer the current conversation's language if it is evident; otherwise fall back to Korean (`ko`).
...
- options: exactly two — (1) yes, star it → then run `bash "setup/setup.sh" star yes`; (2) no thanks → then run `bash "setup/setup.sh" star no`

If the output is empty, just continue silently. Do not narrate beyond the question itself.

# Insane Search

> URL 접근이 차단될 때, **사이트 무관한** 대체 접근 전략을 자동 선택한다. Antigravity CLI, IDE 및 AGY 2.0 호환.

## 하네스 규칙 (Agent에게 강제되는 지침)

이 규칙은 Agent가 즉흥 판단으로 엇나가지 못하게 하기 위한 **고삐**다. 위반 시 이전 test.md 세션처럼 "chrome 200에서 break → safari 미시도 → Playwright 미설치라 포기" 식의 오판이 재현된다.

**R1 — 일반 웹 URL 차단/403/402 감지 시**:
1. WebFetch, 즉흥 curl, 수동 헤더 조합 **시도 금지**
2. 즉시 다음을 실행:
   ```bash
   cd skills/insane-search && python3 -m engine "<URL>" [--selector "<CSS>"] [--device auto|desktop|mobile] [--trace]
   # 또는: PYTHONPATH=skills/insane-search python3 -m engine "<URL>" [--selector "<CSS>"] [--device auto|desktop|mobile] [--trace]
   ```
...
3. `must_invoke_playwright_mcp=false` — true면 **Agent가 세션에서 직접** MCP Playwright를 돌린 뒤에만 통과: `browser_navigate` → `browser_network_requests`로 내부 `/api`·`/graphql`·`.json` 엔드포인트 탐지 → 그 URL을 `python3 -m engine`로 재호출(API는 WAF가 얕음); 또는 `browser_snapshot`으로 렌더된 HTML 회수. (engine은 로컬 Node Chrome만 띄울 수 있고 MCP는 못 돌리므로, MCP는 **구조적으로** 에이전트의 몫이다.)
...
단, R7 조건(WAF 조기 감지)이 성립하면 engine 격자는 계속 돌되, Agent가 **병렬로** MCP 정찰 루트를 시도할 수 있다. 빠른 쪽이 이긴다.
...
**"병렬"의 실행 의미** (Agent 도구 호출이 순차이므로 명확화):
- engine은 `run_in_background=true`로 Bash 툴에서 띄워둔다 — 격자는 그대로 돌되 블로킹하지 않음
- Agent는 그 사이 foreground에서 MCP Playwright 정찰 루트를 진행
...
**R9 — 네이버 검색 키워드 보정 및 분할 방지 규칙 (R-NAV-1)**:
1. **조건부 Fallback/Remediation 실행 원칙**: 쿼리 확장은 무조건 적용하지 않으며, **조건부 fallback 전략**으로만 사용해야 합니다. 에이전트는 먼저 잘 알려진 브랜드나 서비스명에 대해 단순 단일 키워드 쿼리로 검색을 시도해야 합니다.
2. **쿼리 확장 실행 조건**: 다음의 경우에 한해서만 컨텍스트 앵커 접미사를 추가하는 2차 검색(쿼리 확장)을 수행하십시오:
   (a) 1차 단순 단일 키워드 검색 시, 저품질이거나 광고/노이즈로 심하게 오염된 검색 결과를 경험할 때, 또는
   (b) 검색하려는 대상이 등록되지 않은 덜 알려진 고유명사(예: `제로초`)여서 첫 검색부터 형태소 분석기에 의한 분할(splitting) 발생 가능성이 매우 높다고 판단될 때.
3. **접미사 사용 및 금지 지침**:
   - 일반적인 `블로그`, `공식 홈페이지`, `공식`과 같은 범용 접미사는 형태소 분할과 광고 노출에 따른 검색 결과 오염을 더욱 심화시키므로 **단독으로 절대 사용하지 마십시오**.
   - 쿼리 확장을 수행할 때는 대상의 도메인 성격에 따라 다음의 도메인 고정 접미사를 우선적으로 사용합니다:
     - **개발자/인플루언서/강사**: `[키워드] 인프런`, `[키워드] 유튜브`, `[키워드] 깃허브`, `[키워드] 강의`, `[키워드] 자바스크립트`
     - **IT 스타트업/기업**: `[키워드] 테크블로그`, `[키워드] 채용`, `[키워드] 기술블로그`
     - **기타 금융/서비스**: `[키워드] 금융`, `[키워드] 송금` (도메인 비즈니스 기능어 조합)
상세 분석 및 가이드는 `skills/insane-search/references/naver.md`를 참고하십시오.

**R10 — Antigravity 2.0 환경에서의 기본 브라우저 도구 배제**:
1. Antigravity 2.0 환경에서 동작할 때(macOS, Windows, Linux 모두 포함), 기본 시스템 브라우저 도구인 `open_browser_url` (CORTEX_STEP_TYPE_OPEN_BROWSER_URL)은 WAF 탐지 우회(impersonation) 및 Patchright를 통한 CDP 탐지 회피 기능이 결여되어 있으며 플랫폼별 구동 제한이 발생할 수 있습니다.
2. 따라서 웹 페이지 수집이나 브라우저 자동화가 필요할 때는 기본 시스템 브라우저 도구(`open_browser_url` 등)를 호출하지 말고 완전히 배제해야 합니다. 대신, 이 스킬의 자체 우회 로직과 검증 규칙이 적용되는 `insane-search` 실행 경로(예: `python3 -m engine "<URL>"`)를 즉시 선택하거나 Jina Reader 등 다른 적절한 대체 채널을 활용해야 합니다.

**R11 — 서브에이전트 위임 시의 제약 조건 전파 및 배제 규정**:
1. 에이전트가 `browser_subagent`나 다른 subagent를 호출하여 웹 수집 또는 브라우저 조작을 위임할 때, **반드시** subagent의 Task/Prompt 인자에 이 스킬의 제약(예: "WAF 우회가 필요하므로 open_browser_url 사용 금지, 필요 시 engine 모듈 호출 혹은 Jina 사용")을 명시적으로 명문화하여 전달해야 합니다.
2. Subagent가 상속된 제약을 인지하지 못하고 `open_browser_url`을 호출하려다 에러를 뿜는 현상을 예방하기 위해, 호출 전에 Prompt 레벨에서 원천 차단하십시오.
...
`fetch_chain`의 `needs_js_exec only` 케이스는 **Agent 세션에서 MCP 도구를 직접 호출**해야 한다. subprocess 경로 없음. 즉:
1. `result.summary`에 "Playwright MCP must be invoked from the agent session"이 포함되면
2. `mcp__playwright__browser_navigate` → `browser_wait_for` → `browser_snapshot` 흐름으로 Agent가 직접 처리
...
```bash
cd "skills/insane-search/engine/templates" && npm install
...
# 또는 PYTHONPATH 지정: PYTHONPATH=skills/insane-search python3 -m engine "<URL>"
...
이 섹션은 **참조 파일 선택 가이드**다. 문제가 생겼을 때 어떤 `references/*.md`를 열어야 할지 결정하는 기준으로 쓴다. 에이전트(Agent)는 필요할 때만 해당 파일을 `Read`하고, 선제적으로 전부 읽지 않는다.
