...
def _looks_like_xml(text: str, ctype: str) -> bool:
    ...
    while cleaned.startswith("<!--"):
        ...
        if end_idx == -1:
            break
        cleaned = cleaned[end_idx + 3:].strip()
    s = cleaned[:10].lower()
    return s.startswith("<?xml") or s.startswith("<rss") or s.startswith("<feed")


def _xml_ok(text: str) -> Optional[bool]:
    lowered = text.lower()
    ...
    if cleaned.lower().startswith("<?xml"):
        cleaned = re.sub(r'(?i)^<\?xml', '<?xml', cleaned, count=1)
    import xml.etree.ElementTree as ET
    try:
        root = ET.fromstring(cleaned)
    except Exception:
        return None
    if len(root) == 0 and not (root.text and root.text.strip()):
        return False
    return True
    ...
    *,
    ...
    if _looks_like_json(text, ctype):
        ...
        if j is not None:
            ...
            r.verdict = Verdict.CHALLENGE
            r.reasons.append("json_parse_error")
            return r

    # --- Layer 4.5: XML awareness (for XML / RSS / Atom APIs) -------------
    if _looks_like_xml(text, ctype):
        x_ok = _xml_ok(text)
        if x_ok is not None:
            if not (200 <= status <= 299):
                soft = _soft_marker_hits(lowered)
                if soft:
                    ...
                    return r
            r.verdict = Verdict.WEAK_OK
            r.reasons.append("xml_ok")
            return r
        else:
            r.verdict = Verdict.CHALLENGE
            r.reasons.append("xml_parse_error")
            return r
