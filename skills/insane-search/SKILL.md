...
**R10 — macOS 환경에서의 브라우저 도구 제한 및 우회**:
1. macOS 환경에서 실행할 때, 기본 시스템 브라우저 도구인 `open_browser_url` (CORTEX_STEP_TYPE_OPEN_BROWSER_URL)은 `'local chrome mode is only supported on Linux'` 에러 메시지와 함께 실패합니다.
2. 따라서 macOS에서 브라우저 기반 작업을 수행할 때는 `open_browser_url` 도구를 호출하지 않고 완전히 건너뛰어야 하며, 즉시 `insane-search` 실행 경로(예: `python3 -m engine "<URL>"`)를 선택하거나 Jina Reader 등 다른 대체 채널을 사용해야 합니다.
