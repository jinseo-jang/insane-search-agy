...
1. 에이전트가 browser_subagent나 다른 subagent를 호출하여 웹 수집 또는 브라우저 조작을 위임할 때, 반드시 subagent의 Task/Prompt 인자에 이 스킬의 제약(예: "WAF 우회가 필요하므로 open_browser_url 사용 금지, 필요 시 engine 모듈 호출 혹은 Jina 사용")을 명시적으로 명문화하여 전달해야 합니다.
2. Subagent가 상속된 제약을 인지하지 못하고 open_browser_url을 호출하려다 에러를 뿜는 현상을 예방하기 위해, 호출 전에 Prompt 레벨에서 원천 차단하십시오.
